...
export class BackgroundDataSyncService {
  ...
  private LEADERBOARD_SYNC_COOLDOWN = 300000;
  private FULL_SYNC_COOLDOWN = 60000;
  ...
  async start(): Promise<void> {
    if (this.isRunning) {
      logger.info(
        "Background sync service is already running",
    ...
    try {
      logger.info(
        "Starting background data sync service...",
        "BackgroundDataSyncService"
      );
      ...
      logger.info(
        "Background data sync service started successfully",
        "BackgroundDataSyncService"
      );
    } catch (error) {
      logger.error(
        "Failed to start background sync service",
        "BackgroundDataSyncService",
        error
      );
  ...
  stop(): void {
    if (!this.isRunning) {
      logger.info(
        "Background sync service is not running",
        "BackgroundDataSyncService"
      );
      return;
    }
    ...
    logger.info(
      "Stopping background data sync service...",
      "BackgroundDataSyncService"
    );
    ...
    logger.info(
      "Background data sync service stopped",
      "BackgroundDataSyncService"
    );
  }
  ...
  private async performFullSync(): Promise<void> {
    const now = Date.now();
    if (now - this.lastFullSync < this.FULL_SYNC_COOLDOWN) {
      logger.info(
        "Full sync skipped due to cooldown",
        "BackgroundDataSyncService",
        {
          timeSinceLastSync: now - this.lastFullSync,
          cooldownMs: this.FULL_SYNC_COOLDOWN,
        }
      );
      return;
    }
    ...
    try {
      ...
      if (this.syncStatus.syncCount % 5 === 0) {
  ...
  private async getRealTimeMarketData(): Promise<any[]> {
    try {
      logger.info(
        "Fetching real-time market data from CoinGecko...",
        "BackgroundDataSyncService"
      );
      ...
      logger.info(
        `Fetched ${marketData.length} market data entries`,
        "BackgroundDataSyncService"
      );
      ...
      logger.error(
        "Error fetching real-time market data",
        "BackgroundDataSyncService",
        error
      );
      ...
      try {
        logger.info(
          "Attempting to use cached market data as fallback",
          "BackgroundDataSyncService"
        );
        const cachedData = await enhancedCryptoService.getMarketData(
          false,
          100,
          true
        );
        if (cachedData && cachedData.length > 0) {
          logger.info(
            `Using cached market data with ${cachedData.length} entries`,
            "BackgroundDataSyncService"
          );
          return cachedData;
        }
        ...
        logger.error(
          "Failed to get cached market data as fallback",
          "BackgroundDataSyncService",
          fallbackError
        );
      }
  ...
  private async syncPortfolioData(marketData: any[]): Promise<void> {
    try {
      logger.info(
        "Syncing portfolio data with current prices...",
        "BackgroundDataSyncService"
      );
      ...
      const validPortfolios = portfolios.filter(
        (portfolio) =>
          portfolio.symbol && portfolio.symbol.trim() !== "" && portfolio.id
      );

      if (validPortfolios.length === 0) {
        logger.info(
          "No valid portfolio data to sync",
          "BackgroundDataSyncService"
        );
        return;
      }
      ...
      const updates = validPortfolios.map((portfolio) => {
        const currentPrice =
          currentPrices[portfolio.symbol] ||
          parseFloat(portfolio.current_price || "0");
        ...
        const profitLossPercent =
          avgCost > 0 ? (profitLoss / costBasis) * 100 : 0;
      ...
      for (
        let i = 0;
        i < updates.length;
        i += this.config.maxConcurrentUpdates
      ) {
        ...
        if (updateError) {
          logger.error(
            "Error updating portfolio batch",
            "BackgroundDataSyncService",
            updateError
          );
        }
      ...
      logger.info(
        `Updated ${updates.length} portfolio items with real-time prices`,
        "BackgroundDataSyncService"
      );
    } catch (error) {
      logger.error(
        "Error syncing portfolio data",
        "BackgroundDataSyncService",
        error
      );
      throw error;
  ...
  private async syncUserData(): Promise<void> {
    try {
      logger.info(
        "Syncing user data with updated portfolio values...",
        "BackgroundDataSyncService"
      );
      ...
      for (const user of users) {
        try {
          ...
          if (portfolioError) {
            logger.error(
              `Error fetching portfolio for user ${user.id}`,
              "BackgroundDataSyncService",
              portfolioError
            );
            errorCount++;
          ...
          const usdtBalance = parseFloat(
            portfolio.find((p) => p.symbol.toUpperCase() === "USDT")
              ?.total_value || "0"
          );
          ...
          logger.error(
            `Error updating user data for user ${user.id}`,
            "BackgroundDataSyncService",
            error
          );
          errorCount++;
      ...
      logger.info(
        `Updated ${successCount} users, ${errorCount} errors`,
        "BackgroundDataSyncService"
      );
    } catch (error) {
      logger.error(
  ...
  private async syncLeaderboardData(): Promise<void> {
    try {
      ...
      if (now - this.lastLeaderboardSync < this.LEADERBOARD_SYNC_COOLDOWN) {
        logger.info(
          "Leaderboard sync skipped due to cooldown",
          "BackgroundDataSyncService"
        );
        return;
      }
      ...
      logger.info(
        "Leaderboard data synced successfully",
        "BackgroundDataSyncService"
      );
    } catch (error) {
      logger.error(
  ...
  private async cleanupInconsistencies(): Promise<void> {
    try {
      ...
      if (symbolError) {
        logger.error(
          "Error cleaning portfolio entries with null symbols",
          "BackgroundDataSyncService",
          symbolError
        );
      } else {
        logger.info(
          "Cleaned up portfolio entries with null symbols",
          "BackgroundDataSyncService"
        );
      }
      ...
      if (usersError) {
        logger.error(
          "Error fetching users for cleanup",
          "BackgroundDataSyncService",
          usersError
        );
        return;
      ...
      if (!users || users.length === 0) {
        logger.info(
          "No users found, skipping orphaned data cleanup",
          "BackgroundDataSyncService"
        );
        return;
      }
      ...
      if (portfolioFetchError) {
        logger.error(
          "Error fetching portfolio entries for cleanup",
          "BackgroundDataSyncService",
          portfolioFetchError
        );
        ...
        if (orphanedPortfolioIds.length > 0) {
          ...
          if (orphanError) {
            logger.error(
              "Error cleaning orphaned portfolio entries",
              "BackgroundDataSyncService",
              orphanError
            );
          } else {
            logger.info(
              `Cleaned up ${orphanedPortfolioIds.length} orphaned portfolio entries`,
              "BackgroundDataSyncService"
            );
          }
        }
      ...
      const { data: allLeaderboardEntries, error: leaderboardFetchError } =
        await supabase.from("leaderboard_rankings").select("id, user_id");

      if (leaderboardFetchError) {
        logger.error(
          "Error fetching leaderboard entries for cleanup",
          "BackgroundDataSyncService",
        ...
        if (orphanedLeaderboardIds.length > 0) {
          ...
          if (leaderboardError) {
            logger.error(
              "Error cleaning orphaned leaderboard entries",
              "BackgroundDataSyncService",
              leaderboardError
            );
          } else {
            logger.info(
              `Cleaned up ${orphanedLeaderboardIds.length} orphaned leaderboard entries`,
              "BackgroundDataSyncService"
            );
          }
        }
      ...
      const { data: allTransactions, error: transactionFetchError } =
        await supabase.from("transactions").select("id, user_id");

      if (transactionFetchError) {
        logger.error(
          "Error fetching transaction entries for cleanup",
          "BackgroundDataSyncService",
        ...
        if (orphanedTransactionIds.length > 0) {
          ...
          if (transactionError) {
            logger.error(
              "Error cleaning orphaned transaction entries",
              "BackgroundDataSyncService",
              transactionError
            );
          } else {
            logger.info(
              `Cleaned up ${orphanedTransactionIds.length} orphaned transaction entries`,
              "BackgroundDataSyncService"
            );
          }
        }
      ...
      logger.info(
        "Data inconsistencies cleaned up",
        "BackgroundDataSyncService"
      );
    } catch (error) {
      logger.error(
        "Error cleaning up inconsistencies",
        "BackgroundDataSyncService",
        error
      );
      throw error;
  ...
  private async retrySync(): Promise<void> {
    for (let attempt = 1; attempt <= this.config.retryAttempts; attempt++) {
      try {
        logger.info(
          `Retrying sync attempt ${attempt}/${this.config.retryAttempts}`,
          "BackgroundDataSyncService"
        );
  ...
  static resetInstance(): void {
    if (BackgroundDataSyncService.instance) {
      ...
      logger.info(
        "BackgroundDataSyncService singleton instance reset",
        "BackgroundDataSyncService"
      );
    }
  }
